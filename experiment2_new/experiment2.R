...
for (j in 1:iter){
  ...
  for(sig in sigma_vec){
    ...
    output_impULA<-grad_MCMC(T=T,sigma=sig,n=n,method="ImpULA",start_x=start_x)
    #plot(output_UB$x_samples[,1],main=sig,ylab="UB")
...
axis(1, at=1:10,labels=sigma_vec)
legend("topright",c("UB","ULA","impULA"),col=c("black","red","blue"),pch=1)
points(colMeans(sim1_from_truth_ULA),col="red",type="b")
points(colMeans(sim1_from_truth_),col="red",type="b")
